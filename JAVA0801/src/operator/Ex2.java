...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 *          ex2) long + float = (float)long + float = float + float = float // 1+ 3.14 = 4.14(실수)
		 *          
		 */	
		byte b1 = 10; //byte 는 -128 ~ 127 이니까 오류 x
		byte b2 = 20;
		byte b3;
		
		//b1 과 b2 의 연산 결과를 byte 타입 변수 b3 에 저장할 때 문제 발생!
//b3 = b1 + b2; //오류 발생! (byte + byte = int + int = int 이므로,
		//오른쪽은 int(b1 + b2), 왼쪽은 byte 라 오류 난 것)
...
//		short s = b1 + b2; // 오류 발생 ! byte + byte = int + int 이므로
		...
		char ch = 'A';
		//char 타입 변수 ch2에 ch값에 2를 더한 결과를 저장하려고 한다.
...
//	6 7 에 해당하는 문자 'C' (67을 char 타입으로 변환한 데이터)가 저장됨
		System.out.println(ch2);
		System.out.println(ch + 2); //ch='A' -> int(65) + 2 = 67
		//=> ch + 2 의 결과값은 int 형 이므로 문자 'C' 대신 67 출력
		//=> 이 결과값을 다시 char 타입으로 변환하면 문자 'C' 가 출력됨
		System.out.println((char)(ch+2)); //67 을 char 타입으로 변환하여 'C' 출력됨
		...
		// int 형 변수 i2 에 i + l 결과값을 저장하자.
//		int i2 = i + l; //int + long = long 이므로 int 타입 변수에 저장 불가
		int i2 = (int)(i + l); //int 타입으로 강제 형변환
		System.out.println(i2);
		
		float f = 3.14f;
		// => float f = (float)3.14 와 같다.
		long l2 =(long)(l+f); //long + float = float 이므로 long 으로 변환
		System.out.println(l2);
		
		System.out.println("=========================");
		
		System.out.println(3/2); //1.5 라는 결과값 얻고싶으면?
		//나눗셈 연산은 몫을 계산하며, int / int = int 가 되어
		//3 / 2 의 연산 결과는 정수 1이 출력됨
		//만약, 3/2 의 ㅣ결과를 1.5의 결과값으로 얻고 싶을 경우
		//정수가 아닌 실수 형태의 나눗셈 연산으로 연산을 수행해야 한다.
		//따라서, 최소한 피연산자 중 하나를 실수로 변환한 후 연산을 수행
		//=> 규칙 2에 의해서  int 형 보다 큰 데이터 타입을 따라감
...
//		byte b5 = 10 + 200; // byte 타입 범위 (-128~127)을 초과하므로 int 로 취급됨
//		byte b5 = (byte)(10 + 200); 
